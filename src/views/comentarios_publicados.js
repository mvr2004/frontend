...
const PublishedCommentsPage = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get('https://backend-9hij.onrender.com/comentario/publicados');
        setComments(response.data.comentarios);
        setIsLoading(false);
      } catch (err) {
        console.error('Erro ao buscar comentários publicados:', err);
        ...
        <Table striped bordered hover>
          <thead>
            <tr>
              ...
              <th>Comentário</th>
              <th>Data</th>
              <th>Publicado Em</th> {/* Nova coluna para onde foi publicado */}
            </tr>
          </thead>
          <tbody>
...
export default PublishedCommentsPage;
